...
void invalidate_inodes(int dev)
{
	...
	for (i = 0; i < NR_INODE;
		...
		if (inode->i_dev == dev) { // 如果是指定设备的i 节点，则
			if (inode
				->i_count) // 如果其引用数不为0，则显示出错警告；
				printk("inode in use on removed disk\n\r");
			inode->i_dev = inode->i_dirt =
			    0; // 释放该i 节点(置设备号为0 等)。
		}
	}
}
...
{
	struct m_inode * inode;
	static struct m_inode * last_inode =
	    inode_table; // last_inode 指向i 节点表第一项。
	int i;
	...
	do {
	    ...
	    inode
		->i_count); // 如果i 节点又被其它占用的话，则重新寻找空闲i 节点。
...
struct m_inode * get_pipe_inode(void)
{
	...
	if (!(inode = get_empty_inode())) // 如果找不到空闲i 节点则返回NULL。
		return NULL;
	if (!(inode->i_size =
		  get_free_page())) { // 节点的i_size 字段指向缓冲区。
