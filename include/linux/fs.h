...
static inline void _INC_PIPE(unsigned long * head)
{
	__asm__("incl %0\n\tandl $4095,%0" ::"m"(
	    *head)); /* 管道头指针递增，到4095 后回绕。*/
}
...
struct super_block
{
	...
	struct buffer_head *
	    s_imap[8]; // i 节点位图缓冲块指针数组(占用8 块，可表示64M)。
	struct buffer_head *
	    s_zmap[8]; // 逻辑块位图缓冲块指针数组（占用8 块）。
	unsigned short s_dev; // 超级块所在的设备号。
	struct m_inode *
	    s_isup; // 被安装的文件系统根目录的i 节点。(isup-super i)
