...
static inline void ltr(unsigned long n)
{
	__asm__ __volatile__ ("ltr %%ax"::"a" (_TSS(n)));	/* 加载任务寄存器tr。*/
}
// 宏定义，加载第n 个任务的局部描述符表寄存器ldtr。
//#define lldt(n) __asm__( "lldt %%ax":: "a" (_LDT(n)))
static inline void lldt(unsigned long n)
{
	__asm__ __volatile__ ("lldt %%ax"::"a" (_LDT(n)));	/* 加载局部描述符表寄存器ldtr。*/
}
...
static inline void _str(unsigned long n)
{
	...
	*(unsigned long *)n = __tmp;			/* 把当前任务号写回调用者提供的变量。*/
}
/*#define str(n) \
__asm__( "str %%ax\n\t"	// 将任务寄存器中TSS 段的有效地址->ax \
"subl %2,%%eax\n\t"		// (eax - FIRST_TSS_ENTRY*8)->eax \
...
static inline void _set_base(unsigned short *addr,unsigned long base)
{
	__asm__ ("push %%edx\n\t"
		...
		"pop %%edx"
		::"m" (*((char *)addr+2)), "m" (*((char *)addr+4)), "m" (*((char *)addr+7)),
		  "d" (base));
}
/*
__asm__( "movw %%dx,%0\n\t" \
...
static inline void _set_limit(unsigned short *addr,unsigned long limit)
{
	__asm__ ("push %%edx\n\t"
		...
		"movb %%dl,%1\n\t"		/* 并放回[addr+6]处。*/
		"pop %%edx"
		::"m" (*((char *)addr)), "m" (*((char *)addr+6)), "d" (limit));
}
/*
...
static inline unsigned long _get_base(void *addr)
{
...
static inline unsigned long get_limit(unsigned long segment)
{
	...
	__asm__ ("lsll %1,%0\n\tincl %0"
		:"=r" (__limit):"r" (segment));	/* 取段长：lsl 得到的是最后一个有效字节偏移。*/
