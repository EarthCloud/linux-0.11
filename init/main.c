...
/* 必须先包含 stddef.h（unistd.h 里用到了 ptrdiff_t）与 unistd.h，
 * 这样下面的 _syscall0~3 宏才已定义；unistd.h 稍后会再次被包含。 */
#include <stddef.h>
#include <unistd.h>
 ...
 * 但是我们同时还定义了其它一些函数。
 */
/* 前置声明：fork/pause 必须以"始终内联"的方式展开。
 * 原因见下方注释 —— main() 是经 move_to_user_mode() 的 iret 进入的，
 * 栈上没有合法的返回地址，普通函数调用会破坏堆栈。 */
...
static inline _syscall0(int,
			fork) // 是unistd.h 中的内嵌宏代码。以嵌入汇编的形式调用
 ...
 */
	...
	if (memory_end >
	    12 * 1024 * 1024) // 如果内存>12Mb，则设置缓冲区末端=4Mb
...
void init(void)
{
	...
	printf("%d buffers = %d bytes buffer space\n\r",
	       NR_BUFFERS,
	       NR_BUFFERS *
		   BLOCK_SIZE); // 打印缓冲区块数和总字节数，每块1024 字节。
	...
	if (!(pid = fork())) {
		...
		execve("/bin/sh",
		       argv_rc,
		       envp_rc); // 装入/bin/sh 程序并执行。(/lib/execve.c)
		_exit(
		    2); // 若execve()执行失败则退出(出错码2,“文件或目录不存在”)。
	}
