...
#define TSTPMASK \
	(1 << (SIGTSTP - 1)) // tty 发出的停止进程(tty stop)信号屏蔽位。
...
#define _L_FLAG(tty, f) \
	((tty)->termios.c_lflag & f) // 取termios 结构中的本地模式标志。
#define _I_FLAG(tty, f) \
	((tty)->termios.c_iflag & f) // 取termios 结构中的输入模式标志。
#define _O_FLAG(tty, f) \
	((tty)->termios.c_oflag & f) // 取termios 结构中的输出模式标志。
...
#define L_ECHOKE(tty) \
	_L_FLAG((tty), ECHOKE) // 规范模式时，取KILL 擦除行并回显标志位。

// 取termios 结构中输入模式标志中的一个标志位。
#define I_UCLC(tty) \
	_I_FLAG((tty), IUCLC) // 取输入模式标志集中大写到小写转换标志位。
...
#define O_POST(tty) \
	_O_FLAG((tty), OPOST) // 取输出模式标志集中执行输出处理标志。
...
struct tty_struct tty_table[] = {
	...
	{0,
	 ...
	 0,
	 0,
	 ""} /* console secondary queue */ // tty 控制台辅助(第二)队列。
    },
    {
	{0,
...
struct tty_queue * table_list[] = {
    &tty_table[0].read_q,
    &tty_table[0].write_q, // 控制台终端读、写缓冲队列地址。
    &tty_table[1].read_q,
    &tty_table[1].write_q, // 串行口1 终端读、写缓冲队列地址。
    &tty_table[2].read_q,
    &tty_table[2].write_q // 串行口2 终端读、写缓冲队列地址。
};
