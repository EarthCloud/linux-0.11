...
#define _S(nr)        \
	(1 << ((nr) - \
	       1)) // 取信号nr 在信号位图中对应位的二进制数值。信号编号1-32。
// 比如信号5 的位图数值 = 1<<(5-1) = 16 = 00010000b。
#define _BLOCKABLE \
	(~(_S(SIGKILL) | _S(SIGSTOP))) // 除了SIGKILL 和SIGSTOP 信号以外其它都是
...
void show_task(int nr, struct task_struct * p)
{
	...
	while (
	    i < j &&
	    !((char *)(p + 1))[i]) // 检测指定任务数据结构以后等于0 的字节数。
		i++;
...
void show_stat(void)
{
	...
	for (i = 0; i < NR_TASKS;
	     i++) // NR_TASKS 是系统能容纳的最大进程（任务）数量（64 个），
...
union task_union { // 定义任务联合(任务结构成员和stack 字符数组程序成员)。
	struct task_struct
	    task; // 因为一个任务数据结构与其堆栈放在同一内存页中，所以
...
struct task_struct * current =
    &(init_task.task); // 当前任务指针（初始化为初始任务）。
...
unsigned char current_DOR =
    0x0C; // 数字输出寄存器(初值：允许DMA 和请求中断、启动FDC)。
...
int ticks_to_floppy_on(unsigned int nr)
{
	extern unsigned char
	    selected; // 当前选中的软盘号(kernel/blk_drv/floppy.c,122)。
	unsigned char mask =
	    0x10 << nr; // 所选软驱对应数字输出寄存器中启动马达比特位。
...
{
	extern int
	    beepcount; // 扬声器发声时间滴答数(kernel/chr_drv/console.c,697)
	extern void sysbeepstop(
	    void); // 关闭扬声器(kernel/chr_drv/console.c,691)
