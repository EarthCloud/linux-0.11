...
void release(struct task_struct * p)
{
	...
	panic(
	    "trying to release non-existent task"); // 指定任务若不存在则死机。
}
...
static void kill_session(void)
{
	struct task_struct ** p =
	    NR_TASKS + task; // 指针*p 首先指向任务数组最末端。
...
repeat:
	...
	for (p = &LAST_TASK; p > &FIRST_TASK;
		...
		if (!*p || *p == current) // 跳过空项和本进程项。
			continue;
		if ((*p)->father !=
		    current->pid) // 如果不是当前进程的子进程则跳过。
			continue;
		if (pid > 0) { // 如果指定的pid>0，但扫描的进程pid
			if ((*p)->pid != pid) // 与之不等，则跳过。
				continue;
		} else if (!pid) { // 如果指定的pid=0，但扫描的进程组号
			if ((*p)->pgrp !=
			    current->pgrp) // 与当前进程的组号不等，则跳过。
				continue;
		} else if (pid != -1) { // 如果指定的pid<-1，但扫描的进程组号
	...
	if (flag) {		       // 如果子进程没有处于退出或僵死状态，
		if (options & WNOHANG) // 并且options = WNOHANG，则立刻返回。
			return 0;
		current->state =
		    TASK_INTERRUPTIBLE; // 置当前进程为可中断等待状态。
		schedule();		// 重新调度。
		if (!(current->signal &=
		      ~(1 << (SIGCHLD - 1)))) // 又开始执行本进程时，
